...
reclass <- function(df, i, j) {
  df[, "DON_ratio"][df[, "DON_ratio"] <= i] <- "Low"
  df[, "DON_ratio"][df[, "DON_ratio"] < j] <- "Medium"
  df[, "DON_ratio"][(df[, "DON_ratio"] != "Low") & (df[, "DON_ratio"] != "Medium")] <- "High"
  df[, "DON_ratio"] <- factor(df[, "DON_ratio"], levels = c("Low", "Medium", "High"))
  return(df)
}
...
#  M2_test$DON<-log10(M2_test$DON)
  ...
  rf_DON_ratio_m2 <- model_build(WP2Train,"DON_ratio")

## test in testing set
test_rf = predict(rf_DON_ratio_m2, newdata = WP2Test)
## ConfusionMatrix
print(calculateConfusionMatrix(test_rf))
## get the prediction performance
train_rf = predict(rf_DON_ratio_m2, newdata = WP2Train)
